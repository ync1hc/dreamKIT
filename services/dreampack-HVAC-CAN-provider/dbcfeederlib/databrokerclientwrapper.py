...
class DatabrokerClientWrapper(clientwrapper.ClientWrapper):
    ...
    def on_broker_connectivity_change(self, connectivity):
        ...
        # Convert to string
        str_connectivity = str(connectivity)
        log.info("Connectivity as string: %s", str_connectivity)
        if str_connectivity == "ChannelConnectivity.IDLE":
            log.info("Vehicledatabroker restarts for some reasons. Need to restart dbcfeeder.")

            # Restart the application
            log.info("Restarting the application due to IDLE connectivity.")
