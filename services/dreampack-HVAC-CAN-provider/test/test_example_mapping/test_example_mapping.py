...
def test_current_gear():
    dbc_mapping = mapper.get_dbc2vss_mappings("DI_gear")
...
def test_parking_lock():
    ...
    # And now just check some items
    assert mapping_list[0]["from"] == "DI_GEAR_D"
    assert mapping_list[0]["to"] is False
    assert mapping_list[1]["from"] == "DI_GEAR_P"
    assert mapping_list[1]["to"] is True
...
def test_vehicle_speed():
    dbc_mapping = mapper.get_dbc2vss_mappings("DI_uiSpeed")
    assert len(dbc_mapping) == 2
    ...
    assert vss_mapping.transform is None
